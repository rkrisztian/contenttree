...
- Omit the `standalone` property inside Angular decorators, because it already defaults to "true".
...
- Use `NgOptimizedImage` for all static images.
  ...
  - Don't use `NgOptimizedImage` for `mat-icon` components that render glyphs from the Material
    Icons font.
...
- Prefer arrow functions as class methods to prevent issues with `this` overrides.
