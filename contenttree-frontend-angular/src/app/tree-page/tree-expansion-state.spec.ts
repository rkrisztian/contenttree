...
import { TreeNodeRespDTO } from '../api/tree-api.service';
...
describe('TreeExpansionState', () => {
  ...
  describe('isEqual', () => {
    it('should return true for identical states', () => {
      ...
      const otherState = new TreeExpansionState();
      otherState.toggleExpanded(1, treeData);

      expect(expansionState.isEqual(otherState)).toBe(true);
    });
    ...
    it('should return false for different states', () => {
      ...
      expect(expansionState.isEqual(otherState)).toBe(false);
    });
  });
});
