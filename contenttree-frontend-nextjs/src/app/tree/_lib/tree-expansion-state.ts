...
export class TreeExpansionState {
  constructor(
    private readonly collapsedIds: Set<number> = new Set([]),
    private readonly hiddenIds: Set<number> = new Set([]),
  ) {}
  ...
  readonly clone = () => new TreeExpansionState(this.collapsedIds, this.hiddenIds);
}
